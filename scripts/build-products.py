...
LINES = [
    dict(id="rose-hydrangea-bouquet", name="장미수국 꽃다발", cat="꽃다발",
         ...
         recipient=["연인", "배우자"],
         occasion=["기념일", "프러포즈", "생일", "화이트데이", "발렌타인데이", "크리스마스"],
         giftType=["휴대전달형"],
    ...
    dict(id="superior-rose", name="수페리어 장미", cat="꽃다발",
         match=dict(all=["수페리어"]),
         recipient=["연인", "배우자"], occasion=["프러포즈", "기념일", "화이트데이", "발렌타인데이"],
         giftType=["휴대전달형"],
    ...
    dict(id="single-stem-soap-box", name="비누꽃 한송이·두송이 박스", cat="소품",
         ...
         occasion=["스승의날", "소소한 감사", "단체·답례품",
                   "화이트데이", "발렌타인데이", "빼빼로데이", "병문안"],
         giftType=["휴대전달형", "단체·답례품"],
         situation="행사·모임에서 여러 명에게 하나씩 돌리거나, 부담 없이 마음만 가볍게 전할 때",
         reason="1만원 안쪽 가격에 투명 박스까지 포함이라 답례품·단체 선물은 물론, 낱개라 병문안처럼 짐이 되지 않아야 하는 자리에도 부담 없어요."),
